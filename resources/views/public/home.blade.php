<x-app-layout>
  {{-- styles (can live once per page) --}}

  <x-slot name="header">
    <!-- Elegant, interactive hero (no heavy animations) -->
    <section id="hero" class="relative mx-0 md:mx-6 mb-10 overflow-hidden rounded-3xl shadow-xl text-white" style="--mx:50%;--my:30%">
      <!-- Layered background: base gradient + soft glows + cursor spotlight + subtle vignette -->
      <div class="pointer-events-none absolute inset-0">
        ...
        {{-- hairline inner border removed --}}
      </div> <!-- ✅ close the background stack OUTSIDE the Blade comment -->

      <!-- Content (put it above the background) -->
      <div class="relative z-10 px-6 py-20 sm:px-10 md:py-24">
        <div class="mx-auto max-w-4xl text-center">
          <div class="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold backdrop-blur-sm">
            <span>📚 EduShare</span>
            <span class="opacity-80">Study better, together</span>
          </div>
        </div>

        <!-- Glass card around the main message (no border) -->
        <div class="mx-auto mt-6 max-w-3xl rounded-2xl bg-transparent shadow-none px-6 py-10 backdrop-blur-md shadow-lg">
          <div class="text-center">
            <h1 class="text-4xl md:text-5xl font-extrabold leading-tight tracking-tight drop-shadow-sm">
              Unlock Knowledge, <span class="opacity-90">Share Insights</span>
            </h1>

            <p class="mt-4 text-lg leading-7 text-indigo-100/90">
              Find, upload, and discuss notes across faculties and semesters.
            </p>

            <!-- Search -->
            <form action="{{ route('public.notes.index') }}" method="GET" class="mx-auto mt-8 max-w-2xl">
              <div class="flex items-stretch gap-3">
                <div class="relative flex-1">
                  ...
                  <input
                    ...
                    class="w-full rounded-xl border border-white/20 bg-white/95 pl-10 pr-3 py-3.5 text-base text-gray-900 shadow
                ...
                <button
                  type="submit"
                  class="shrink-0 rounded-xl bg-indigo-600 px-6 py-3.5 text-sm font-semibold text-white
            ...
            <div class="mt-6 flex flex-wrap items-center justify-center">
              <a href="{{ route('public.notes.index') }}" class="rounded-full border border-white/25 bg-white/10 px-4 py-1.5 text-sm font-medium text-white hover:bg-white/15 backdrop-blur-sm transition">
                Browse All Notes →
              </a>
              @auth
                <a href="{{ route('notes.create') }}" class="ml-3 mt-2 sm:mt-0 rounded-full bg-white px-4 py-1.5 text-sm font-semibold text-indigo-700 hover:bg-indigo-50 transition">
                  Upload a Note
                </a>
              @endauth
            </div>
  ...
  {{-- ====================== BODY STACK (even vertical rhythm) ====================== --}}
  <div class="space-y-12 md:space-y-16">
    ...
    {{-- ========== HOME FEED (tabs + grid, live update) ========== --}}
    <section
      x-data="homeFeed({
    ...
    >
      ...
      <div class="mt-6 mb-4 flex items-end justify-between">
        ...
        <a :href="viewAllHref()" class="text-sm font-medium text-indigo-600 hover:text-indigo-700">
          View all →
        </a>
      </div>

      {{-- Grid target (server renders initial grid; Alpine swaps it on tab/faculty change) --}}
  ...
  <script>
    function homeFeed({ baseUrl, initial }) {
      return {
        ...
        setFaculty(id) {
          this.faculty_id = id === this.faculty_id ? '' : id; // toggle off if clicked again
          this.update();
        },
        ...
        async update() {
          ...
          try {
            ...
            // Try to locate a grid in the index partial response
            ...
            if (grid) {
              this.$refs.feed.innerHTML = grid.outerHTML;
            } else {
              // Fallback to full navigation if server didn't return partials
              window.location.href = this.viewAllHref();
              return;
            }
          } catch (e) {
            window.location.href = this.viewAllHref(); // degrade gracefully
          } finally {
            this.loading = false;
          }
        },
